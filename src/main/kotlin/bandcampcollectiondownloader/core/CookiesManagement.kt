...
class CookiesManagement(private val util: Util) {
  ...
  private data class ParsedCookie(
      @SerializedName("Name raw") val nameRaw: String?,
      @SerializedName("Content raw") val contentRaw: String?
  )

  data class Cookies(val source: Path, val content: Map<String, String>)
    ...
    val parsedCookies =
        try {
          if (cookiesFile.toString().endsWith(".json"))
              parsedCookiesToMap(gson.fromJson(fileData, Array<ParsedCookie>::class.java))
          else parseCookiesText(fileData)
        } catch (e: JsonSyntaxException) {
          throw BandCampDownloaderError(
              "Cookies file '$cookiesFile' is not well formed: ${e.message}")
        }
    return Cookies(cookiesFile, parsedCookies)
  }

  /**
   * Searches in all Firefox profiles for valid Bandcamp cookies, and returns all found sets of
   * cookies.
   */
  fun retrieveFirefoxCookies(): List<Cookies> {
    ...
    val pathKey = "Path"
    val entriesWithPath = ini.keys.filter { ini[it] != null && ini[it]!!.containsKey(pathKey) }
        ...
        try {
          ...
          while (rs.next()) {
            ...
            if (!originAttributes.contains("userContextId")) {
              ...
              if (difference > 0) result[name] = value
            }
          }
        } finally {
          connection?.close()
