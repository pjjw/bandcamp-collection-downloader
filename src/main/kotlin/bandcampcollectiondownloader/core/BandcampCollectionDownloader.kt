...
class BandcampCollectionDownloader(
  ...
  fun downloadAll() {
    ...
    for (cookies in cookiesCandidates) {
      try {
        ...
        logger.log(
            """Found "$pageName" with ${candidateConnector.getAllSaleItemIDs().size} items.""")
    ...
    if (connector == null) {
      throw BandCampDownloaderError(
          "Could not connect to the Bandcamp API with the provided cookies.")
    }
    ...
    val itemsToDownload =
        connector.getAllSaleItemIDs().filter { saleItemId -> saleItemId !in cacheContent }
  ...
  private fun manageDownloadPage(
      connector: BandcampAPIConnector,
      saleItemId: String,
      cache: Cache
  ) {
    ...
    // If null, then the download page is simply invalid and not usable anymore, therefore it can be
    // added to the cache
    if (digitalItem == null) {
      logger.log(
          "Sale Item ID $saleItemId cannot be downloaded anymore (maybe a refund?); skipping")
    ...
    val isSingleTrack: Boolean =
        (digitalItem.download_type == "t" ||
            digitalItem.download_type_str == "track" ||
    ...
    connector.retrieveRealDownloadURL(saleItemId, args.audioFormat)
        ?: throw BandCampDownloaderError(
            "No URL found for item (maybe the release has no digital item, or the provided download format is invalid)")
    ...
    when {
      Files.isRegularFile(downloadFolderPath) ->
          throw BandCampDownloaderError(
              message.format(
                  "download", downloadFolderPath.toAbsolutePath().normalize().toString()))
      Files.isRegularFile(artistFolderPath) ->
          throw BandCampDownloaderError(
              message.format("artist", artistFolderPath.toAbsolutePath().normalize().toString()))
      Files.isRegularFile(releaseFolderPath) ->
          throw BandCampDownloaderError(
              message.format("release", releaseFolderPath.toAbsolutePath().normalize().toString()))
    }
    ...
    if (Files.isDirectory(downloadFolderPath) && !Files.isDirectory(artistFolderPath)) {
      val candidateArtistFolders =
          Files.list(downloadFolderPath)
      ...
      if (candidateArtistFolders.isNotEmpty()) {
        ...
        val candidateReleaseFolders =
            Files.list(artistFolderPath)
                .filter { f ->
                  Files.isDirectory(f) && f.fileName.toString().endsWith(releaseFolderName, true)
                }
                .collect(Collectors.toList())
        releaseFolderPath =
        ...
        logger.log(
            "Using existing folder found with different case: " +
  ...
  private fun downloadRelease(
    ...
    val amountFiles =
        if (!args.dryRun || Files.exists(releaseFolderPath))
            releaseFolderPath.toFile().listFiles()!!.size
        else 0
    if (amountFiles < 2) {
