...
constructor(
  ...
  private val saleItemsIDs2digitalItems: MutableMap<String, DigitalItem?> =
      HashMap<String, DigitalItem?>()
  ...
  private data class FanData(val fan_id: String)
  ...
  private data class ParsedStatDownload(val download_url: String?, val url: String)
  ...
  fun init() {
    ...
    if (!initialized) {
      ...
      if (collection.isEmpty()) {
        throw BandCampDownloaderError(
            "No download links could by found in the collection page. This can be caused by an outdated or invalid cookies file.")
      }
      ...
      if (fanPageBlob.collection_data.item_count > fanPageBlob.collection_data.batch_size) {
        collection.putAll(
            retrieveDownloadURLs(
                fanPageBlob.fan_data.fan_id, fanPageBlob.collection_data, "collection_items"))
      }

      // If needed, get the rest of the hidden collection
      if ((!skipHiddenItems) &&
          (fanPageBlob.hidden_data.item_count > fanPageBlob.hidden_data.batch_size)) {
        collection.putAll(
            retrieveDownloadURLs(
                fanPageBlob.fan_data.fan_id, fanPageBlob.hidden_data, "hidden_items"))
      }
  ...
  fun retrieveDigitalItemData(saleItemID: String): DigitalItem? {
    ...
    if (!this.saleItemsIDs2digitalItems.containsKey(saleItemID)) {
      ...
      util.retry(
          {
            try {
              val downloadPage = Jsoup.connect(saleItemURL).cookies(cookies).timeout(timeout).get()
              ...
              // If 404, then the download page is not usable anymore for some reason (eg. a refund
              // was given for the purchase)
              if (e.statusCode == 404) this.saleItemsIDs2digitalItems[saleItemID] = null
              else throw e
            }
          },
          retries)
  ...
  fun retrieveRealDownloadURL(saleItemID: String, audioFormat: String): String? {
    ...
    // Some releases have no digital items (eg. vinyl only) or no downloads or no urls, so we return
    // null in such cases
    ...
    val statdownloadURL: String =
        downloadUrl.replace("/download/", "/statdownload/").replace("http:", "https:") +
            "&.vrs=1" +
            "&.rand=" +
            random.nextInt()

    // Get statdownload JSON
    val statedownloadUglyBody: String =
        util.retry(
            {
    ...
    val prefixPattern =
        Pattern.compile("""if\s*\(\s*window\.Downloads\s*\)\s*\{\s*Downloads\.statResult\s*\(\s*""")
    val suffixPattern = Pattern.compile("""\s*\)\s*};""")
    val statdownloadJSON: String =
        prefixPattern
            .matcher(suffixPattern.matcher(statedownloadUglyBody).replaceAll(""))
            .replaceAll("")
